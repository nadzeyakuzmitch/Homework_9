...
@router.post("/qr-codes/", response_model=QRCodeResponse, status_code=status.HTTP_201_CREATED, tags=["QR Codes"]) # NADZEYA FIX, type and incorrect code as mentioned on line 23
async def create_qr_code(request: QRCodeRequest, token: str = Depends(oauth2_scheme)):
    ...
    if qr_code_full_path.exists():
        ...
        return JSONResponse(
            status_code=status.HTTP_409_CONFLICT, # NADZEYA FIX, incorrect HTTP code type, as mentioned on line 43
...
@router.delete("/qr-codes/{qr_filename}", status_code=status.HTTP_204_NO_CONTENT, tags=["QR Codes"]) # NADZEYA FIX, typo, filename, and supposed to be 204 as mentioned on line 78
